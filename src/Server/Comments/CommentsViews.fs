...
module Views =
  let index (ctx : HttpContext) (objs : Comment list) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Listing Comments"]
          ...
          tbody [] [
            for o in objs do
              yield tr [] [
                ...
                td [] [rawText (string o.article_id)]
        ...
        a [_class "button is-text"; _href (Links.add ctx )] [rawText "New Comment"]
      ]
    ]
    App.layout ctx ([section [_class "section"] cnt])


  let show (ctx : HttpContext) (o : Comment) =
    let cnt = [
      div [_class "container "] [
        h2 [ _class "title"] [rawText "Show Comment"]

        ul [] [
          li [] [ strong [] [rawText "Id: "]; rawText (string o.id) ]
          li [] [ strong [] [rawText "Author_id: "]; rawText (string o.author_id) ]
          li [] [ strong [] [rawText "Article_id: "]; rawText (string o.article_id) ]
  ...
  let private form (ctx: HttpContext) (o: Comment option) (validationResult : Map<string, string>) isUpdate =
  ...
  let add (ctx: HttpContext) (o: Comment option) (validationResult : Map<string, string>)=
    form ctx o validationResult false

  let edit (ctx: HttpContext) (o: Comment) (validationResult : Map<string, string>) =
